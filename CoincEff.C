...
void FinalCoincEff() {
   ...
   if(OUTPUT_EFF) {EffOut.open("CoincEffOut.txt");}
   ...
   for(Clover=0;Clover<CLOVERS;Clover++) {
      ...
      dEff = Eff * sqrt(pow(dCounts/Counts,2) + pow(sqrt(hTestSpectrum->GetBinContent(4))/hTestSpectrum->GetBinContent(4),2));
      if(OUTPUT_EFF) {
